...
const SidebarLayoutSettings: React.FC<SidebarLayoutSettingsProps> = ({
  ...
  return (
    <div className="flex flex-col overflow-hidden transition-all duration-500 border text-[var(--text-main)]" style={{
      ...
      <div className="flex-1 overflow-y-auto overflow-x-hidden custom-scrollbar" style={{
        ...
        <div className="space-y-4">
          ...
          {layout.useGrid !== false && (
            <div
              className="p-4 mt-2 space-y-4 rounded-[1.25rem] border overflow-hidden relative group transition-all duration-300 bg-transparent shadow-sm glass-item"
              style={{ borderColor: 'var(--widget-picker-pill-tint-active)' }}
            >
              <div className="absolute top-0 left-0 w-1 h-full bg-[var(--primary-color)]" style={{ opacity: 0.4 }} />
              ...
              <div className="flex items-center gap-2 p-1 bg-transparent rounded-xl border glass-item" style={{ borderColor: 'var(--widget-picker-icon-border)' }}>
                <div className="flex-1 flex flex-col gap-1 px-2 py-1.5 group/input transition-all">
                  ...
                  <div className="flex items-center gap-1.5">
                    <LayoutGrid className="w-3 h-3 text-[var(--text-muted-icon-dim)]" />
                    <input
                      type="number" min="1" max={layout.columns}
                      value={batchW}
                      onChange={(e) => setBatchW(parseInt(e.target.value, 10) || 1)}
                      className="w-full bg-transparent text-sm font-mono font-bold text-main outline-none"
                      style={{ '::placeholder': { color: 'var(--text-muted-icon-faint)' } } as any}
                      placeholder="W"
                    />
                  </div>
                </div>
                ...
                <div className="flex-1 flex flex-col gap-1 px-2 py-1.5 group/input transition-all">
                  ...
                  <div className="flex items-center gap-1.5">
                    <Layers className="w-3 h-3 text-[var(--text-muted-icon-dim)]" />
                    <input
              ...
              <button
                ...
                className="w-full group/btn relative overflow-hidden py-2.5 px-4 rounded-xl flex items-center justify-center gap-2 transition-all duration-300 active:scale-95 shadow-lg bg-[var(--primary-color)] text-xs font-bold hover:brightness-110"
                style={{ color: 'var(--white)' }}
              >
                <div className="absolute inset-0 w-full h-full transform -translate-x-full group-hover/btn:anim-shimmer-2s bg-gradient-to-r from-transparent via-[var(--widget-picker-shimmer-color)] to-transparent pointer-events-none" />

                <CheckCircle2 className="w-4 h-4 transition-transform group-hover/btn:scale-110" style={{ color: 'var(--white)' }} />
                <span>일괄 적용하기</span>
              </button>

              <div className="flex items-start gap-1.5 px-1 py-0.5">
                <div className="w-1 h-1 rounded-full mt-1.5 shrink-0 bg-[var(--primary-color)]" style={{ opacity: 0.4 }} />
          ...
          >
            <div className="flex items-center gap-2">
              <div className="w-8 h-8 shrink-0 rounded-lg bg-transparent border border-[var(--border-muted)] flex items-center justify-center p-1" title="위로 쏠리지 않고 고정">
                <svg viewBox="0 0 20 20" className="w-full h-full text-primary" style={{ opacity: 0.7 }}>
                  <rect x="1" y="2" width="18" height="5" fill="currentColor" opacity="0.4" rx="0.5" />
                  <rect x="3" y="9" width="6" height="5" fill="currentColor" opacity="0.35" rx="0.5" />
                  <rect x="11" y="9" width="6" height="5" fill="currentColor" opacity="0.35" rx="0.5" />
                </svg>
              </div>
